...
type Link = Option<RefCell<Rc<Token>>>;
...
impl Neighborhood {
    fn add(&mut self, token: &Rc<Token>>, pos: NeighborPosition) {
    ...
    fn get(&mut self, pos: NeighborPosition) -> Option<Token> {
        match pos {
            ...
            NeighborPosition::CenterRight => if let Some(a) = &self.center_right { 
                ...
                // return a;
                let token = a.take(); 
                println!("rc count for {}: {}", token.value, Rc::strong_count(a));
