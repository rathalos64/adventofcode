...
impl Token {
    ...
    fn has_symbol_adjacent(&self) -> bool {
    ...
    fn is_part_number(&self) -> bool {
        ...
        self.has_symbol_adjacent() ||
            self.get_neighbor(NeighborPosition::CenterRight).unwrap_or_default().is_part_number()
    }

    fn get_part_number(&self) -> Result<i32, String> {
        if !self.is_begin() {
...
pub fn run(input_file: &String) -> Result<i32, Box<dyn std::error::Error>> {
    ...
    let mut tokens: Vec<Rc<Token>> = Vec::new();
    tokens.push(Rc::new(Token::new("3".to_owned())));
    tokens.push(Rc::new(Token::new("4".to_owned())));
    tokens.push(Rc::new(Token::new("#".to_owned())));
    ...
    println!("{}", tokens[0].is_part_number());
    println!("{}", tokens[0].get_part_number()?);
    // println!("tokens[0] == {:?}", tokens[0]);
    // println!("tokens[1] == {:?}", tokens[1]);
    // println!("tokens[2] == {:?}", tokens[2]);
