...
#[derive(Eq, PartialEq, Debug, Hash, EnumIter, Clone, Copy)]
...
pub fn run(input_file: &String) -> Result<(i64, i64), Box<dyn std::error::Error>> {
    ...
    for seed in seeds.iter() {
        ...
        let min = ranges.par_iter().min_by_key(|x| x.from).ok_or(std::fmt::Error)?;
        min_locations_all.push(*min);
    }
    println!("min locations all: {:?}", min_locations_all);
    let min_all = min_locations_all.par_iter().min_by_key(|x| x.from).ok_or(std::fmt::Error)?;
    ...
    for seed in all_seeds_raw.iter() {
        ...
        // store as mutex to prevent race conditions
        // https://stackoverflow.com/a/30560208
        // https://static.rust-lang.org/doc/master/std/sync/struct.Mutex.html
        let locations: &Arc<Mutex<Vec<Range>>> = &Arc::new(Mutex::new(Vec::new()));
        let shadow_almanac = almanac.clone(); // needs a clone to move into the closure

        // weird behaviour in solution finding
        // use 1000000000 and the result will be 49094938
        // use 100000000 and the result will be 47909639 (which is the correct one)
        // 
        // same with fractions of the to range
        // 9th = 47909639
        // 8th = 49094938
        
        // really cool using into_par_iter() of the rayon package
        bucket_range(*seed, seed.to / 9).into_par_iter().try_for_each(move |bucket| -> Result<_, std::fmt::Error> {
            let found = find_locations(bucket, &shadow_almanac)?; // store in variable makes it inherently faster
            locations.lock().unwrap().extend(found);
            Ok(())
        })?;    
        let ranges = locations.lock().unwrap().clone(); // must unlock to get the result and clone as it's moved into the closure
    
        let sum_diff_ranges = ranges.par_iter().map(|r| (r.from - r.to).abs()).sum::<i64>() + (ranges.len() as i64) - 1; 
    ...
    let min_tuple = min_locations_tuple.par_iter().min_by_key(|x| x.from).ok_or(std::fmt::Error)?;
...
fn bucket_range(range: Range, n: i64) -> Vec<Range> {
    let mut buckets: Vec<Range> = Vec::new();
    let mut from = range.from;
    while from <= range.to {
        buckets.push(Range{from: from, to: cmp::min(from + n - 1, range.to)});
        from = cmp::min(from + n, range.to + 1);
    }
    println!("bucket original, from: {}, to: {}", range.from, range.to);
    for bucket in buckets.iter() {
        println!("bucket from: {}, to: {}", bucket.from, bucket.to);
    };
    buckets
}

fn find_locations(seed: Range, almanac: &HashMap<AlmanacType, Vec<Mapping>>) -> Result<Vec<Range>, std::fmt::Error> {
...
fn test_bucket_range() {
    let observed_1 = bucket_range(Range{from: 1, to: 5}, 2);
    assert_eq!(observed_1.len(), 3); 
    assert_eq!(observed_1[0], Range{from: 1, to: 2}); assert_eq!(observed_1[1], Range{from: 3, to: 4}); assert_eq!(observed_1[2], Range{from: 5, to: 5});

    let observed_2 = bucket_range(Range{from: 1, to: 6}, 2);
    assert_eq!(observed_2.len(), 3);
    assert_eq!(observed_2[0], Range{from: 1, to: 2}); assert_eq!(observed_2[1], Range{from: 3, to: 4}); assert_eq!(observed_2[2], Range{from: 5, to: 6});
}
